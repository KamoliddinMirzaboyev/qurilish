...
import { useAdminMines, useDeleteMine } from "@/features/mines/hooks";
...
export default function AdminMinesPage() {
  ...
  const { data, isLoading } = useAdminMines(page);
  const deleteMutation = useDeleteMine();
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {isLoading ? (
            ...
            <Card key={mine.id} className="flex flex-wrap items-center justify-between gap-4">
              <div className="min-w-[220px] flex-1">
                ...
                <p className="mt-1 text-sm text-ink-muted">
                  {mine.location} · {mine.rawMaterialType} · {mine.volume}
                </p>
              </div>
        ...
        <EmptyState title="Hozircha konlar joylashtirilmagan." />
      )}
